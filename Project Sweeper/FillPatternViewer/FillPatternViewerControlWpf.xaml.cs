...
namespace PKHL.ProjectSweeper.FillPatternViewer
{
    ...
    /// Source:thebuildingcoder.typepad.com/blog/2014/04/wpf-fill-pattern-viewer-control.html
    /// </summary>
    public partial class FillPatternViewer : INotifyPropertyChanged, IDisposable
    {
        ...
        {
            var fillPatternViewerControl = d as FillPatternViewer;
        ...
        #region UserScalar DependencyProperty
        public static readonly DependencyProperty
            UserScalarProperty = DependencyProperty
            .RegisterAttached("UserScalar",
                              typeof(double),
                              typeof(FillPatternViewer),
                              new UIPropertyMetadata(1.0d, OnUserScalarChanged));


        private static void OnUserScalarChanged(
            DependencyObject d,
        ...
        public double UserScalar
        {
            ...
            set
            {
                SetValue(UserScalarProperty, value);
            }
        }

        public double GetUserScalar(
        ...
        public void SetUserScalar(
            DependencyObject obj,
            double value)
        {
            obj.SetValue(UserScalarProperty, value);
        }
        #endregion
        ...
        private void DrawFillPattern(Graphics g)
        {
            ...
            if (fillPattern.Target == FillPatternTarget.Model)  //check segment density here?
                matrixScale = _scale * (float)UserScalar;
            else
                matrixScale = _scale * 100 * (float)UserScalar;
            ...
            try
            {
                ...
                foreach (var fillGrid in fillGrids)
                {
                    ...
                    while (moving_forward || moving_back)    //draw segments shifting and offsetting each time
                    {
                        ...
                        {
                            g.Transform = matrix;
                            g.DrawLine(pen, new PointF(0, 0), new PointF(_length, 0));
                        }
                        else
                        {
